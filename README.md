...
- **签到自动补签**：自动签到时检查当月漏签，消耗补签卡自动补上（默认关闭，受米游社补签限额约束）
- **便笺提醒增强**：树脂提醒阈值可调（80/120/160/200）；可选"游戏运行时免打扰"（检测原神前台，退出后重新提醒）
- **UIAF 剪贴板导入**：成就记录支持从剪贴板直接粘贴 UIAF 数据导入

### 新增功能（1.8.5）

- **签到记录页**：侧边栏查看当前角色当月签到进度、漏签天数、补签卡余额与每日奖励一览

### 修复（1.8.6）

- **桌面组件添加即引导配置**：从桌面添加组件时自动打开配置界面（标准configure流程），保存后自动渲染,不再出现添加后一直显示"未配置"的问题;未配置占位文案改为"点击配置组件";移除组件背景上的调试水印
